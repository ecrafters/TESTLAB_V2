...
import { login, getHospitalName, navigateToPatientsList, createPatientWithInsurer, getFirstPatientFromAPI } from '../utils/patient-helpers';
...
test('Facturer les prestations', async ({ page }) => {
    ...
    await test.step('TC-026 : Facturer une consultation avec un patient assuré', async () => {
        // await page.pause(); // Pause pour permettre l'inspection manuelle avant de naviguer vers les prestations
...
async function createPrestationAmbulatoire(page: Page, patientName: string) {
    ...
    await page.waitForResponse('**/dokploy-medical-billing/1.0/prestations/rev2');
